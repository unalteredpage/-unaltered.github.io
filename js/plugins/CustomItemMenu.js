...
(function() {
    ...
    Window_CustomItemList.prototype.processCursorMove = function() {
        // 아이템칸 또는 USE 줄에서 왼쪽 키 → 물품 메뉴로 복귀.
        // (INFO 줄에서 왼쪽은 기존대로 USE 줄로 이동하도록 남겨둠)
        if (this.isOpenAndActive() && Input.isRepeated('left') &&
            this.index() !== this.infoIndex()) {
            Input.update();                    // 이 왼쪽 입력을 소비 (물품버튼이 재감지 못하게)
    ...
    Window_CustomItemList.prototype.cursorDown = function(wrap) {
        var i = this.index();
        if (i < CFG.ITEM_ROWS) {
            // 아이템칸 안: 다음 채워진 칸이 있을 때만 이동.
            // 마지막 아이템칸에서 아래키로 USE 줄로는 내려가지 않음 (엔터로만 가능).
