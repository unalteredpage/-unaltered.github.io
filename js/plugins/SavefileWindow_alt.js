//=============================================================================
// SavefileWindow_alt.js
//=============================================================================
// 맵에서 병렬실행(플러그인 호출)으로 사용하는 세이브파일 스타일 윈도우
// = 실제 타이틀 화면을 대체하는 "가짜 타이틀" (변수 초기화 방지 목적)
//
// alt 버전: "정체불명의 종이 확인"/"리셋" 버튼 제거, "계속하기" 버튼 추가.
// 구성: 이름 / LV / 플레이타임 / 옛날집(힌트N/리셋수N) / 버튼 2개
//   버튼: 계속하기 | 설정
//=============================================================================

/*:
 * @plugindesc 세이브파일 스타일 정보 윈도우 (alt) - 계속하기/설정만
 * @author Claude
 *
 * @help
 * 플러그인 명령:
 *   ShowSavefileWindowAlt  - 윈도우를 표시하고 입력을 활성화합니다 (플레이타임 정지)
 *   HideSavefileWindowAlt  - 윈도우를 숨기고 비활성화합니다 (플레이타임 재개)
 *
 * 변수:
 *   0005번 - 힌트를 본 횟수
 *   0006번 - 클리어(리셋) 횟수
...
(function() {
    ...
    // 텔레포트 대상 (계속하기)
    var CONTINUE_MAP_ID = 2; // 맵 002
    var CONTINUE_X      = 2; // X 좌표
    var CONTINUE_Y      = 4; // Y 좌표

    // 버튼 정의: 라벨 + 동작 타입. 순서가 곧 좌→우 배치 순서.
    var BUTTON_DEFS = [
        { label: '계속하기', type: 'continue' },
    ...
    var _persistentState = {
        visible:     false, // ShowSavefileWindowAlt ~ HideSavefileWindowAlt 사이인지
    ...
    Window_SavefileInfo.prototype._executeButton = function(index) {
        ...
        switch (def.type) {
            ...
            case 'continue':
                // 창을 닫고 맵 002번의 (2,4) 좌표로 이동
                unfreezePlaytime();
                _persistentState.visible     = false;
                _persistentState.optionsOpen = false;
                this.deactivateWindow();
                this.visible = false;
                // 플레이어를 아래 방향(2)을 바라보게 하며 이동. 페이드 전환 포함.
                $gamePlayer.reserveTransfer(CONTINUE_MAP_ID, CONTINUE_X, CONTINUE_Y, 2, 0);
                break;
        }
    };
    ...
    //-------------------------------------------------------------------------
    // 터치/마우스 클릭 판정
    //  TouchInput.x/y 는 MV 가 이미 캔버스 스케일/오프셋을 보정해 제공하는
    //  게임 내부 좌표(boxWidth 기준)이다. 따라서 추가 변환 없이
    //  창 위치(this.x/y)와 패딩(this.padding)만 빼면 컨텐츠 로컬 좌표가 된다.
    //-------------------------------------------------------------------------
    Window_SavefileInfo.prototype._processTouch = function() {
        if (!this._buttonRects) return;

        // 컨텐츠 영역 기준 로컬 좌표
        var tx = TouchInput.x - this.x - this.padding;
        var ty = TouchInput.y - this.y - this.padding;
    ...
    Window_FakeOptions.prototype._processTouch = function() {
        // TouchInput.x/y 는 이미 게임 내부 좌표 → 창 위치+패딩만 빼면 로컬 좌표
        var tx = TouchInput.x - this.x - this.padding;
        var ty = TouchInput.y - this.y - this.padding;
    ...
    Scene_Map.prototype.createAllWindows = function() {
        ...
        // 메인(가짜 타이틀) 창
        this._savefileWindowAlt = new Window_SavefileInfo();
        this._savefileWindowAlt.visible = _persistentState.visible;

        // 가상 설정 창 (메인 창 위에 겹쳐 표시)
        this._fakeOptionsWindowAlt = new Window_FakeOptions();
        this._fakeOptionsWindowAlt.setOwner(this._savefileWindowAlt);
        this._savefileWindowAlt._optionsWindow = this._fakeOptionsWindowAlt;
        ...
        if (_persistentState.visible) {
            if (_persistentState.optionsOpen) {
                ...
                this._fakeOptionsWindowAlt.activateWindow();
            } else if (_persistentState.active) {
                this._savefileWindowAlt.activateWindow(_persistentState.cursorIndex);
            }
        }

        this.addChild(this._savefileWindowAlt);
        this.addChild(this._fakeOptionsWindowAlt);
    };
    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        ...
        switch (command) {
            case 'ShowSavefileWindowAlt':
                if (SceneManager._scene && SceneManager._scene._savefileWindowAlt) {
                    freezePlaytime();
                    SceneManager._scene._savefileWindowAlt.visible = true;
                    SceneManager._scene._savefileWindowAlt.activateWindow();
                    _persistentState.visible = true;
                }
                break;

            case 'HideSavefileWindowAlt':
                if (SceneManager._scene && SceneManager._scene._savefileWindowAlt) {
                    var scn = SceneManager._scene;
                    // 설정 창이 열려 있으면 함께 닫는다
                    if (scn._fakeOptionsWindowAlt) {
                        scn._fakeOptionsWindowAlt.deactivateWindow();
                    }
                    scn._savefileWindowAlt.deactivateWindow();
                    scn._savefileWindowAlt.visible = false;
                    unfreezePlaytime();
                    _persistentState.visible     = false;
                    _persistentState.optionsOpen = false;
                }
